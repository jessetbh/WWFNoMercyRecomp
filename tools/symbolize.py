#!/usr/bin/env python3
"""Symbolize crash-backtrace RVAs against build-msvc/NoMercyRecompiled.map.
...
`NoMercyRecompiled.exe +0xRVA` (module-relative), so match on
...
MAP = ROOT / "build-msvc" / "NoMercyRecompiled.map"
BASE = 0x140000000

SYM_RE = re.compile(
    r"^\s*[0-9a-f]{4}:[0-9a-f]{8}\s+(\S+)\s+([0-9a-f]{16})\s", re.IGNORECASE)
FRAME_RE = re.compile(r"NoMercyRecompiled\.exe\s+\+0x([0-9A-Fa-f]+)")
